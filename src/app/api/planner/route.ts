...
async function getTeacherClass(userId: string, classId?: string | null) {
  if (!classId) return null;
  const [row] = await d1Query<ClassRow>(
    "SELECT id, teacher_id, name FROM classes WHERE id = ? AND teacher_id = ? AND is_active = 1 LIMIT 1",
    [classId, userId],
  );
  return row ?? null;
}
    ...
    const [announcements, events] = await Promise.all([
      ...
      d1Query(
        `SELECT e.*, c.name AS class_name, l.title AS lesson_title
          ...
          WHERE e.owner_id = ?
             OR e.class_id IN (SELECT id FROM classes WHERE teacher_id = ?)
...
export async function POST(request: Request) {
  ...
  const classRow = await getTeacherClass(user.id, body.classId);
