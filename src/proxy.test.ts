...
describe("proxy", () => {
  it("redirects protected routes to login when no session exists", () => {
    const response = proxy(makeRequest("/studio"));
  ...
  it("does not send malformed role sessions to the student portal", () => {
    ...
    const response = proxy(makeRequest("/admin/dashboard?tab=ai", cookie));
    expect(response.status).toBe(307);
  ...
  it("routes known non-admin roles away from admin pages", () => {
    ...
    const response = proxy(makeRequest("/admin/dashboard", cookie));
    expect(response.status).toBe(307);
  ...
  it.each([
    ...
    const response = proxy(makeRequest(path, cookie));
