...
import { d1Query, loadCloudflareEnv } from "./lib/cloudflare-d1";
...
async function main() {
  ...
  const existing = await d1Query<AuthUserIdRow>("SELECT id FROM auth_users WHERE lower(email) = lower(?) LIMIT 1", [email]);
